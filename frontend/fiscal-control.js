(function () {
  ...
  function renderInvoiceRows(list) {
    ...
    return list.map(function (invoice) {
      ...
      var danfe = '<button class="fiscal-icon-btn" onclick="downloadFiscalDanfe(' + invoice.id + ')" title="Baixar DANFE da NF-e">' + icon('download') + '</button>';
      var xml = invoice.xml_url ? '<button class="fiscal-icon-btn fiscal-icon-btn-xml" onclick="downloadFiscalXml(' + invoice.id + ')" title="Baixar XML oficial da NF-e">' + icon('download') + '</button>' : '';
  ...
  window.openFiscalInvoiceDetails = async function (id) {
    ...
    try {
      ...
      openModal('<div class="p-6 fiscal-modal-wide fiscal-detail-modal"><div class="fiscal-panel-head px-0 pt-0"><div><h2 class="font-display text-xl font-bold text-imec-dark">NF-e ' + esc(invoice.number || invoice.id) + '</h2><p class="text-sm text-slate-500 mt-1">' + esc(supplier) + ' &bull; emiss&atilde;o ' + dt(invoice.issue_date) + '</p></div>' + statusChip(invoice.status) + '</div>'
        + '<div class="fiscal-action-strip"><button class="btn btn-primary btn-sm" onclick="downloadFiscalDanfe(' + invoice.id + ')">' + icon('download') + ' DANFE da NF-e</button>' + (invoice.xml_url ? '<button class="btn btn-outline btn-sm" onclick="downloadFiscalXml(' + invoice.id + ')">' + icon('download') + ' XML oficial da NF-e</button>' : '') + '<button class="btn btn-outline btn-sm" onclick="editFiscalInvoice(' + invoice.id + ')">' + icon('edit') + ' Editar</button></div>'
        + '<div class="fiscal-official-note"><strong>Nota oficial:</strong> a NF-e juridica e o arquivo XML. A DANFE em PDF e o espelho para conferencia, impressao e controle interno.</div>'
