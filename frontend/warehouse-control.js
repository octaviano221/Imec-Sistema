(function () {
  ...
  function itemCards(items) {
    ...
    if (!list.length) return '<div class="warehouse-empty">Cadastre os principais materiais, EPIs e consumiveis da empresa.</div>';
  ...
  function renderWarehouse() {
    ...
    return '<div class="warehouse-page fade-in">'
      + '<section class="warehouse-hero"><div><p class="warehouse-kicker">controle interno</p><h2>Almoxarifado e compras</h2><p>Fornecedores, materiais, pedidos de compra, anexos e recebimento em uma fila unica para a equipe registrar tudo.</p></div><div class="warehouse-actions">'
      ...
      + kpi('Itens cadastrados', items.length, 'catalogo interno', 'box', '#168844', '#dcfce7')
      ...
      + '<div class="warehouse-table-wrap"><table class="warehouse-table" id="warehouseOrdersTable"><thead><tr><th>Pedido</th><th>Fornecedor</th><th>Centro de custo</th><th>Datas</th><th>Status</th><th>Total</th><th>Acoes</th></tr></thead><tbody>' + orderRows(orders) + '</tbody></table></div></section>'
      + '<aside class="warehouse-panel"><div class="warehouse-panel-head"><div><h2>Estoque critico</h2><p>Itens abaixo do minimo ou principais materiais.</p></div><button class="btn btn-outline btn-sm" onclick="editStockItem()">Adicionar item</button></div><div class="warehouse-side-list">' + itemCards(items) + '</div></aside></div>'
      + '<section class="warehouse-panel"><div class="warehouse-panel-head"><div><h2>Fornecedores cadastrados</h2><p>Empresas usadas em pedidos de compra, servicos e materiais.</p></div><button class="btn btn-outline btn-sm" onclick="editSupplier()">Novo fornecedor</button></div><div class="warehouse-table-wrap"><table class="warehouse-table"><thead><tr><th>Empresa</th><th>Contato</th><th>Telefone</th><th>Status</th><th>Acoes</th></tr></thead><tbody>' + supplierRows(suppliers) + '</tbody></table></div></section>'
      + '</div>';
  }
  ...
  window.editSupplier = function (id) {
    ...
    var html = '<div class="p-6"><h2 class="font-display text-xl font-bold text-imec-dark mb-2">' + (id ? 'Editar fornecedor' : 'Novo fornecedor') + '</h2><p class="text-sm text-slate-500 mb-5">Cadastre empresas para pedidos de compra e historico do almoxarifado.</p><form onsubmit="saveSupplier(event,' + (id || 'null') + ')"><div class="warehouse-form-grid">'
      ...
      + '<div class="warehouse-form-full"><label class="label">Endereco</label><input class="input" id="whSupplierAddress" value="' + esc(supplier.address) + '"></div>'
      + '<div class="warehouse-form-full"><label class="label">Observacoes</label><textarea class="input" id="whSupplierNotes" rows="3">' + esc(supplier.notes) + '</textarea></div></div><div class="flex justify-end gap-3 mt-6"><button type="button" class="btn btn-outline" onclick="closeModal()">Cancelar</button><button class="btn btn-primary">Salvar fornecedor</button></div></form></div>';
    openModal(html);
  };
  ...
  window.editStockItem = function (id) {
    ...
    var html = '<div class="p-6"><h2 class="font-display text-xl font-bold text-imec-dark mb-2">' + (id ? 'Editar item' : 'Novo item') + '</h2><p class="text-sm text-slate-500 mb-5">Controle materiais, EPIs, ferramentas e consumiveis.</p><form onsubmit="saveStockItem(event,' + (id || 'null') + ')"><div class="warehouse-form-grid">'
      ...
      + '<div><label class="label">Estoque atual</label><input type="number" step="0.01" class="input" id="whItemStock" value="' + esc(item.current_stock || 0) + '"></div><div><label class="label">Estoque minimo</label><input type="number" step="0.01" class="input" id="whItemMin" value="' + esc(item.minimum_stock || 0) + '"></div>'
      ...
      + '<div class="warehouse-form-full"><label class="label">Observacoes</label><textarea class="input" id="whItemNotes" rows="3">' + esc(item.notes) + '</textarea></div></div><div class="flex justify-end gap-3 mt-6"><button type="button" class="btn btn-outline" onclick="closeModal()">Cancelar</button><button class="btn btn-primary">Salvar item</button></div></form></div>';
    openModal(html);
  };
  ...
  window.editPurchaseOrder = function (id) {
    ...
    editingOrderLines = 0;
    var html = '<div class="p-6 warehouse-modal-wide"><h2 class="font-display text-xl font-bold text-imec-dark mb-2">' + (id ? 'Editar pedido de compra' : 'Novo pedido de compra') + '</h2><p class="text-sm text-slate-500 mb-5">Registre a solicitacao, fornecedor, itens, valores e anexo do pedido.</p><form onsubmit="savePurchaseOrder(event,' + (id || 'null') + ')"><div class="warehouse-form-grid">'
      ...
      + '<div><label class="label">Solicitante</label><input class="input" id="whOrderRequester" value="' + esc(order.requester_name) + '"></div><div><label class="label">Setor / Centro de custo</label><input class="input" id="whOrderDepartment" value="' + esc(order.department || '') + '" placeholder="Almoxarifado, obra, manutencao"></div>'
